...
resource "aws_organizations_organizational_unit" "level_1_ous" {
  for_each = { for elem in local.level_1_ou_arguments : elem.name => elem }
...
locals {
  ...
  all_ou_attributes = concat(
    ...
    [{
      id        = aws_organizations_organization.organization.roots[0].id,
      arn       = aws_organizations_organization.organization.roots[0].arn,
      parent_id = ""
      name      = aws_organizations_organization.organization.roots[0].name,
    }]
  )
}